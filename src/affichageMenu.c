...
/**
 * @brief Cette fonction permet d'afficher le score final du mode capture de pièce
 * @author Tom MARSURA
 * @return void
*/
...
/*****************************************************************************************************************/
    /*Message*/
    SDL_Surface *Message_crash = TTF_RenderText_Solid(police, "FIN DU CHRONO", TextColor);
    if (Message_crash == NULL)
    {
        TTF_CloseFont(police);
        // SDL_DestroyTexture(texture);
        ...
        SDL_Rect DistMessage = {(SCREEN_WIDTH - Message_crash->w) / 2, 30, Message_crash->w, Message_crash->h};
